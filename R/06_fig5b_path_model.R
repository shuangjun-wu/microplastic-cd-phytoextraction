# Figure 5b: regression-based path model
# Fits the path model linking treatment, soil Cd availability, growth/allocation, shoot Cd and total Cd extraction.
...
# User settings
fig5_data_dir <- path_from_env("FIG5_DATA_DIR", file.path(repo_dir, "data", "fig5"))
panel_a_dir <- file.path(fig5_data_dir, "panel_a")
panel_b_dir <- file.path(fig5_data_dir, "panel_b")
out_dir <- path_from_env("FIG5B_OUT_DIR", file.path(repo_dir, "outputs", "fig5", "panel_b"))
...
# Read input file
search_dirs <- unique(c(panel_b_dir, panel_a_dir, getwd(), file.path(repo_dir, "data")))
...
readr::write_csv(pca_loadings, file.path(out_dir, "fig5b_biomass_pc1_loadings.csv"))

readr::write_csv(
  analysis_df %>% dplyr::select(sample_original, treatment, biomass_allocation_PC1),
  file.path(out_dir, "fig5b_biomass_pc1_scores.csv")
)
...
readr::write_csv(analysis_table, file.path(out_dir, "fig5b_analysis_table.csv"))
...
readr::write_csv(path_tbl, file.path(out_dir, "fig5b_paths.csv"))
...
readr::write_csv(indirect_tbl, file.path(out_dir, "fig5b_indirect_effects.csv"))
...
pdf_file <- file.path(out_dir, "fig5b_path_model.pdf")
png_file <- file.path(out_dir, "fig5b_path_model.png")
