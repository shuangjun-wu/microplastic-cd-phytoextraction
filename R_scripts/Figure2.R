# Figure 2 source-data script
# Generates the Fig. 2 panels and associated statistical summaries.
...
# ---------- fixed path ----------
data_dir <- getwd()
csv_file <- file.path(data_dir, "Source_Data_Fig2.csv")
if (!file.exists(csv_file)) csv_file <- file.path(data_dir, "fig2.csv")
...
if (has_photo) {
  ...
  final_plot <- p_a / middle_row / p_f +
    plot_layout(heights = c(0.68, 1.56, 0.78), widths = c(1, 1))
} else {
  middle_row <- (p_b / p_c) | (p_d / p_e)
  final_plot <- middle_row / p_f +
    plot_layout(heights = c(1.56, 0.78), widths = c(1, 1))
}

save_plot_pair(final_plot, "Fig2_combined_relayout_logic_caps_units_ticks", width_mm = 245, height_mm = 210)
