# Figure 5a source-data script
# Generates the representative multilayer association network.
...
# Fixed plant endpoints for this figure
plant_endpoints <- c(
  "Plant: SDW",
  "Plant: RDW",
  "Plant: SCd"
...
# Read input files
# -------------------------
search_dirs <- unique(c(base_dir, getwd(), "/mnt/data"))
...
# -------------------------
# 1) Nominate class/function nodes from fixed plant endpoints
# -------------------------
...
readr::write_csv(mc_nom$summary, file.path(base_dir, "fig5a_v5_mc_nomination.csv"))
readr::write_csv(mf_nom$summary, file.path(base_dir, "fig5a_v5_mf_nomination.csv"))
...
readr::write_csv(pathway_score_long, file.path(base_dir, "fig5a_v5_tp_scores.csv"))

# pathway -> fixed plant endpoints
...
readr::write_csv(pathway_stats, file.path(base_dir, "fig5a_v5_tp_nomination.csv"))
...
# 5) Node table
# -------------------------
node_names <- sort(unique(c(edge_final$from, edge_final$to)))
...
node_stats <- purrr::map_dfr(node_names, function(nn) {
  x <- edge_final %>% dplyr::filter(from == nn | to == nn)
  tibble(
    node_name = nn,
    degree = nrow(x),
...
readr::write_csv(nodes_tbl, file.path(base_dir, "fig5a_v5_nodes.csv"))
readr::write_csv(edges_tbl, file.path(base_dir, "fig5a_v5_edges.csv"))
...
pdf_file <- file.path(base_dir, "Fig5a_v5i_NCfinal_polish.pdf")
png_file <- file.path(base_dir, "Fig5a_v5i_NCfinal_polish.png")
...
message("Nodes written to: ", file.path(base_dir, "fig5a_v5_nodes.csv"))
message("Edges written to: ", file.path(base_dir, "fig5a_v5_edges.csv"))
